...
GEDE <- function(Y, Est="auto", X=NULL, predictors=seq(1:ncol(Y)), HD=FALSE, HD.iter=5, nMAD=2, ...) {
  if (identical(Est,"auto")) Est <- RobEst(Y, X=X, HD=HD, HD.iter=HD.iter, nMAD=nMAD, ...)
  Tk <- Est$Tk; Lk <- Est$Lk
  ...
  suppressWarnings(Y.imputed <- EigenImpute(Est, Y.out, X=X, predictors=predictors, HD=HD, HD.iter=HD.iter))
...
## and test data
Enhancer <- function(train, test, X.train=NULL, X.test=NULL, method=c("GEDE", "lasso", "lasso2"), predictors=seq(1:ncol(train)), mc.cores=2, ...) {
  method <- match.arg(method)
  train <- as.matrix(train); test <- as.matrix(test)
  n <- nrow(train); m <- ncol(train)
  if (method=="GEDE"){
    Est <- RobEst(train, X=X.train, ...)
    rr <- GEDE(test, Est=Est, X=X.test, predictors=predictors, ...)
  } else if (method=="lasso") {
    Ystar <- Reduce(cbind, mclapply(1:m, function(i) {
      Pred.idx <- setdiff(predictors, i)
      Predtrain <- cbind(train[,Pred.idx], X.train)
      mod.i <- cv.glmnet(x=Predtrain, y=train[,i], type.measure="mse",
                         alpha=1, nfolds=5)
      Predtest <- cbind(test[,Pred.idx], X.test)
      predict(mod.i, s=mod.i$lambda.min, newx=Predtest)
    }, mc.cores=mc.cores))
    rr <- list(Ystar=Ystar)
    ...
    Ystar <- Reduce(cbind, mclapply(1:m, function(i) {
      Pred.idx <- setdiff(predictors, i)
      Predtrain <- cbind(train[,Pred.idx], X.train)
      mod.i <- glmnet(x=Predtrain, y=train[,i])
      ...
      Predtest <- cbind(test[,Pred.idx], X.test)
      predict(mod.i, s=best.lambda, newx=Predtest)
    }, mc.cores=mc.cores))
    rr <- list(Ystar=Ystar)
  } else {
